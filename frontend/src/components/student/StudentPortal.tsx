...
interface StudentPortalProps { initialSection?: StudentSectionId; }
...
export default function StudentPortal({ initialSection = 'dashboard' }: StudentPortalProps) {
    ...
    queryFn: async () => {
      try {
        ...
        if (user?.id && isNetworkOnlyFailure(error)) {
          ...
          if (snapshot) { setUsingOfflineSnapshot(true); return snapshot; }
        }
        throw error;
      }
  ...
  useEffect(() => {
    if (dashboardStatus === 401 || dashboardStatus === 403) { router.replace('/login'); return; }
  ...
  switch (section) {
    ...
    case 'library': content = <LibrarySection />; break;
    case 'ptm': content = <PtmSection />; break;
  ...
  if (dashboardQuery.isLoading || dashboardStatus === 404) return <div className={styles.shell}><GlobalTopbar /><div className={styles.loading}>{dashboardStatus === 404 ? 'Opening Student account setup…' : 'Loading your Student workspace…'}</div></div>;
  if (dashboardQuery.isError) return <div className={styles.shell}><GlobalTopbar /><div className={styles.loading}><div><div className={styles.error}>{apiErrorText(dashboardQuery.error, 'Could not load the Student workspace.')}</div><button className={styles.primary} onClick={() => dashboardQuery.refetch()}>Retry</button></div></div></div>;

  return <div className={styles.shell}>
    <GlobalTopbar />
    {usingOfflineSnapshot && <div style={{ margin: '10px 18px 0', padding: '10px 14px', borderRadius: 10, background: '#FFF7E8', border: '1px solid #F7D79A', color: '#6B4A0B', display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}><span>📶 You are viewing the last saved Student dashboard. Saved lessons and queued learning progress remain available in Offline Mode.</span><button className={styles.secondary} onClick={() => goSection('offline')}>Open Offline Mode</button></div>}
    <div className={styles.layout}>
      <aside className={`${styles.sidebar} ${sidebarOpen ? styles.sidebarOpen : ''}`}>
        ...
        <div className={styles.profile}><div className={styles.avatar}>{student?.name?.toLowerCase().includes('priya') ? '👧' : '👦'}</div><div className={styles.profileName}>{student?.name || user?.name || 'Student'}</div><div className={styles.profileMeta}>{student?.classLabel || `Class ${student?.gradeLevel || '—'}`}{student?.schoolName ? ` · ${student.schoolName}` : ''}</div><div className={styles.xpPill} style={{ background: '#EEF4FF', color: '#0D1B3E' }}>ID: {student?.studentCode || '—'}</div>{student?.schoolLinkStatus === 'PENDING' && <div className={styles.profileMeta} style={{ marginTop: 7 }}>⏳ School approval pending</div>}{student?.schoolLinkStatus === 'APPROVED' && <div className={styles.profileMeta} style={{ marginTop: 7 }}>✅ School verified</div>}</div>
        <nav className={styles.sideNav} aria-label="Student module">{MENU.map(([id, icon, label]) => <button key={id} className={`${styles.sideButton} ${section === id ? styles.sideButtonActive : ''}`} onClick={() => goSection(id)}><span className={styles.sideIcon}>{icon}</span>{label}</button>)}</nav>
      </aside>
