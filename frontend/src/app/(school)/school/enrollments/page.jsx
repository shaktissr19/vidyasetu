...
import { getClasses, getEnrollmentRequests, reviewEnrollmentRequest } from '@/services/schoolService';
...
export default function SchoolEnrollmentRequestsPage() {
  ...
  const requestsQuery = useQuery({
    ...
    queryFn: async () => data(await getEnrollmentRequests(status)) || [],
  });
  const classesQuery = useQuery({
    queryKey: ['school-classes'],
    queryFn: async () => data(await getClasses()) || [],
  });
  ...
  const review = useMutation({
    mutationFn: ({ requestId, action }) => reviewEnrollmentRequest(requestId, {
      action,
    ...
    onError: error => setMessage(err(error)),
  });
  ...
  return (
    <div className="animate-fade-up">
      ...
      {requestsQuery.isError && <div className="card p-6" style={{ color: '#C62828' }}>{err(requestsQuery.error)}</div>}
              ...
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="font-display font-bold text-lg" style={{ color: 'var(--navy)' }}>{request.name}</h2>
                    <span className="text-xs font-bold px-2 py-1 rounded-full" style={{ background: request.status === 'APPROVED' ? '#E8F5E9' : request.status === 'REJECTED' ? '#FFEBEE' : '#FFF8E1', color: request.status === 'APPROVED' ? '#138808' : request.status === 'REJECTED' ? '#C62828' : '#7A5A00' }}>{request.status}</span>
                  </div>
                ...
                {request.status === 'PENDING' && (
                  <div className="w-full lg:w-[320px] p-4 rounded-xl" style={{ background: '#F7F8FA' }}>
                    ...
                    <select className="input select mb-3" value={targetClass} onChange={e => setSelectedClass(v => ({ ...v, [request.id]: e.target.value }))}>
                      {eligibleClasses.map(row => <option value={row.id} key={row.id}>Class {row.class_name}-{row.section} · {row.academic_year}</option>)}
                    </select>
                    <label className="block text-xs font-bold mb-1">Roll Number <span style={{ color: 'var(--slate)', fontWeight: 400 }}>(optional)</span></label>
                    <input className="input mb-3" value={rollNumber[request.id] || ''} onChange={e => setRollNumber(v => ({ ...v, [request.id]: e.target.value }))} placeholder="e.g. 8A27" />
                    <div className="grid grid-cols-2 gap-2">
                      <button className="btn-primary justify-center" disabled={review.isPending || !targetClass} onClick={() => { setSelectedClass(v => ({ ...v, [request.id]: targetClass })); review.mutate({ requestId: request.id, action: 'APPROVE' }); }}>Approve</button>
