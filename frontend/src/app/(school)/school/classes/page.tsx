...
export default function SchoolClassesPage() {
  ...
  const archive = useMutation({
    mutationFn: (classId: string) => archiveClass(classId),
  ...
  function startEdit(row: SchoolClassRow) {
    setEditing(row);
    setForm({ className: row.class_name, section: row.section, roomNumber: row.room_number || '' });
    setShowForm(true);
  }

  return <div className="animate-fade-up">
    <SectionHeader title={`🏷️ ${t('कक्षाएँ और सेक्शन', 'Classes & Sections')}`} sub={`${rows.filter((row) => row.is_active).length} ${t('सक्रिय कक्षाएँ', 'active class-sections')}`}>
