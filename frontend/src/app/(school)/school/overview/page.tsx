...
export default function SchoolOverview() {
  ...
  const quickLinks: Array<[string, string, string, string | number]> = [
  ...
  return (
    <div className="animate-fade-up">
      <SectionHeader title={`🏫 ${school.name || 'School Dashboard'}`} sub={`UDISE: ${school.udise_code || '—'} · ${school.academic_year || '—'} · ${school.district || ''}${school.state ? `, ${school.state}` : ''}`}>
        <button className="btn-primary" onClick={() => router.push('/school/announcements')}>📢 {t('घोषणा भेजें', 'New Announcement')}</button>
      </SectionHeader>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-5">
        <div className="card">
          ...
          {classes.length === 0 ? <p className="text-sm" style={{ color: 'var(--slate)' }}>No active classes found.</p> : <div className="space-y-3">{classes.map((cls) => { const total = Number(cls.total || 0); const present = Number(cls.present || 0); const pct = total > 0 ? Math.round((present / total) * 100) : 0; return <div key={cls.id}><div className="flex justify-between text-sm mb-1"><span className="font-semibold" style={{ color: 'var(--navy)' }}>Class {cls.class_name}{cls.section ? `-${cls.section}` : ''}</span><span style={{ color: pct >= 85 ? 'var(--forest)' : 'var(--saffron)' }}>{present}/{total} ({pct}%)</span></div><ProgressBar pct={pct} color={pct >= 85 ? 'var(--forest)' : 'var(--saffron)'} height={6} showPct={false} /></div>; })}</div>}
        </div>

        <div className="card">
          <div className="flex items-center justify-between mb-4"><h3 className="font-display font-bold text-base" style={{ color: 'var(--navy)' }}>💰 {t('फीस स्थिति', 'Fee Status')}</h3>{canAdmin && <button className="text-xs font-bold" style={{ color: 'var(--saffron)' }} onClick={() => router.push('/school/fees')}>Manage Fees →</button>}</div>
          <div className="space-y-3"><div className="flex items-center justify-between p-3 rounded-xl" style={{ background: 'var(--forest-pale)' }}><span className="font-semibold text-sm">✅ {t('भुगतान', 'Paid')}</span><span className="font-display font-extrabold text-lg" style={{ color: 'var(--forest)' }}>{fee.paid_count || 0}</span></div><div className="flex items-center justify-between p-3 rounded-xl" style={{ background: 'var(--saffron-pale)' }}><span className="font-semibold text-sm">⏳ {t('लंबित', 'Pending')}</span><span className="font-display font-extrabold text-lg" style={{ color: 'var(--saffron)' }}>{fee.pending_count || 0}</span></div>{canAdmin && <button className="btn-primary w-full justify-center mt-2" disabled={reminderMut.isPending} onClick={() => reminderMut.mutate()}>{reminderMut.isPending ? 'Sending…' : `📲 ${t('रिमाइंडर भेजें', 'Send Fee Reminders')}`}</button>}</div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="card"><div className="flex items-center justify-between mb-3"><h3 className="font-display font-bold" style={{ color: 'var(--navy)' }}>🚀 Setup Readiness</h3><span className="font-bold" style={{ color: onboarding.isComplete ? 'var(--forest)' : 'var(--saffron)' }}>{onboarding.completed || 0}/{onboarding.total || 5}</span></div><div className="space-y-2">{Object.entries(onboarding.checks || {}).map(([key, done]) => <div key={key} className="flex justify-between text-sm"><span className="capitalize">{key}</span><span style={{ color: done ? 'var(--forest)' : 'var(--saffron)', fontWeight: 700 }}>{done ? '✓ Ready' : '○ Pending'}</span></div>)}</div>{canAdmin && !onboarding.isComplete && <button className="btn-outline w-full justify-center mt-4" onClick={() => router.push('/school/profile')}>Complete School Setup</button>}</div>
      </div>
    </div>
  );
}
