...
export default function SchoolOverview() {
  ...
  const teacherQuickLinks: Array<[string, string, string, string | number]> = [
    ['/school/academic-workspace', '🎯', 'Academic Workspace', 'Open'],
    ['/school/learning-insights', '🧭', 'Learning Insights', 'Open'],
    ['/school/attendance', '📅', 'Student Attendance', 'Open'],
    ['/school/homework', '📚', 'Homework', 'Open'],
  ];
  const quickLinks = canAdmin ? adminQuickLinks : teacherQuickLinks;

  return (
    <div className="animate-fade-up">
      <SectionHeader title={`🏫 ${school.name || 'School Dashboard'}`} sub={`UDISE: ${school.udise_code || '—'} · ${school.academic_year || '—'} · ${school.district || ''}${school.state ? `, ${school.state}` : ''}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 stagger">
        <StatCard label={t('कुल छात्र', 'Total Students')} value={Number(stats.total_students || 0).toLocaleString()} sub={canAdmin ? `${stats.pending_enrollment_requests || 0} pending enrollment` : t('स्कूल में सक्रिय', 'active in School')} accent="var(--saffron)" />
        ...
        {canAdmin ? <StatCard label={t('फीस संग्रह', 'Fees Collected')} value={formatCurrency(fee.collected || 0)} sub={`${fee.pending_count || 0} pending invoice(s)`} accent="var(--navy)" /> : <StatCard label={t('सक्रिय कक्षाएँ', 'Active Classes')} value={stats.total_classes || 0} sub={t('सीखने और उपस्थिति के लिए', 'for Learning and attendance')} accent="var(--navy)" />}
        <StatCard label={t('शिक्षक', 'Teachers')} value={stats.total_teachers || 0} sub={canAdmin ? `${stats.total_classes || 0} active classes` : t('स्कूल टीम', 'School teaching team')} accent="var(--gold)" />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-5">
        <div className="card">
          <div className="flex items-center justify-between mb-4"><h3 className="font-display font-bold text-base" style={{ color: 'var(--navy)' }}>📅 {t('कक्षावार उपस्थिति', 'Class-wise Attendance Today')}</h3><button className="text-xs font-bold" style={{ color: 'var(--saffron)' }} onClick={() => router.push('/school/attendance')}>Open Attendance →</button></div>
          {classes.length === 0 ? <p className="text-sm" style={{ color: 'var(--slate)' }}>No active classes found.</p> : <div className="space-y-3">{classes.map((cls) => { const total = Number(cls.total || 0); const present = Number(cls.present || 0); const pct = total > 0 ? Math.round((present / total) * 100) : 0; return <div key={cls.id}><div className="flex justify-between text-sm mb-1"><span className="font-semibold" style={{ color: 'var(--navy)' }}>{schoolGradeLabel(cls.class_name, lang)}{cls.section ? `-${cls.section}` : ''}</span><span style={{ color: pct >= 85 ? 'var(--forest)' : 'var(--saffron)' }}>{present}/{total} ({pct}%)</span></div><ProgressBar pct={pct} color={pct >= 85 ? 'var(--forest)' : 'var(--saffron)'} height={6} showPct={false} /></div>; })}</div>}
        </div>
        ...
        {canAdmin ? <div className="card">
          ...
          <h3 className="font-display font-bold text-base" style={{ color: 'var(--navy)' }}>🎯 {t('शिक्षण और लर्निंग हेल्थ', 'Teaching & Learning Health')}</h3>
          <p className="text-sm mt-2" style={{ color: 'var(--slate)' }}>{t('अपनी असाइन की गई कक्षाओं में कॉन्सेप्ट मास्टरी, रिव्यू संकेत और विद्यार्थी सहायता देखें।', 'Review concept mastery, review signals and Student support for your assigned classes.')}</p>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        {canAdmin ? <div className="card"><div className="flex items-center justify-between mb-3"><h3 className="font-display font-bold" style={{ color: 'var(--navy)' }}>🚀 Setup Readiness</h3><span className="font-bold" style={{ color: onboarding.isComplete ? 'var(--forest)' : 'var(--saffron)' }}>{onboarding.completed || 0}/{onboarding.total || 5}</span></div><div className="space-y-2">{Object.entries(onboarding.checks || {}).map(([key, done]) => <div key={key} className="flex justify-between text-sm"><span className="capitalize">{key}</span><span style={{ color: done ? 'var(--forest)' : 'var(--saffron)', fontWeight: 700 }}>{done ? '✓ Ready' : '○ Pending'}</span></div>)}</div>{!onboarding.isComplete && <button className="btn-outline w-full justify-center mt-4" onClick={() => router.push('/school/profile')}>Complete School Setup</button>}</div> : <div className="card"><h3 className="font-display font-bold mb-3" style={{ color: 'var(--navy)' }}>📚 {t('शिक्षक शॉर्टकट', 'Teacher Shortcuts')}</h3><div className="grid grid-cols-2 gap-2"><button className="btn-outline justify-center" onClick={() => router.push('/school/homework')}>📚 Homework</button><button className="btn-outline justify-center" onClick={() => router.push('/school/ptm')}>👥 PTM</button><button className="btn-outline justify-center" onClick={() => router.push('/school/absence')}>🗓️ Absence</button><button className="btn-outline justify-center" onClick={() => router.push('/school/announcements')}>📢 Notices</button></div></div>}
      </div>
    </div>
  );
}
