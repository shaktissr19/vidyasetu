...
const TYPE_LABEL: Record<string, { hi: string; en: string }> = {
  VIDEO: { hi: 'देखें', en: 'Watch' },
  PDF: { hi: 'पढ़ें', en: 'Read' },
  QUIZ: { hi: 'प्रयास करें', en: 'Attempt' },
  NOTES: { hi: 'पढ़ें', en: 'Read' },
  AUDIO: { hi: 'सुनें', en: 'Listen' },
};
...
export default function SubjectDetailPage() {
  ...
  useEffect(() => {
    ...
    if (requestedChapter) setOpenChapter(requestedChapter);
  }, []);

  const { data: chapters = [], isLoading } = useQuery({
    queryKey: ['chapters', subjectId, className],
    queryFn: () => getChapters(subjectId, className).then(r => r.data.data),
  });

  const { data: items = [], isFetching: itemsLoading } = useQuery({
    queryKey: ['content-items', openChapter, lang],
    queryFn: () => openChapter ? getContentItems(openChapter, lang).then(r => r.data.data) : Promise.resolve([]),
  ...
  const progressMut = useMutation({
    ...
    onSuccess: async () => {
      toast.success(t('+10 XP मिला 🎉', '+10 XP 🎉'));
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <h1 className="font-display font-extrabold text-2xl" style={{ color: 'var(--navy)' }}>
            📚 {subjectInfo ? t(subjectInfo.subject_name_hi || '', subjectInfo.subject_name || subjectInfo.title) : t('विषय', 'Subject')}
          </h1>
                ...
                <button
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '0.72rem', color: 'var(--slate)', marginTop: 3 }}>
                      {Number(ch.video_count || 0) > 0 && `🎬 ${ch.video_count} ${t('वीडियो', 'videos')} `}
                      {Number(ch.pdf_count || 0) > 0 && `📄 ${ch.pdf_count} ${t('PDF', 'PDFs')} `}
                      {Number(ch.quiz_count || 0) > 0 && `📝 ${ch.quiz_count} ${t('क्विज़', 'quizzes')}`}
                    </p>
                  </div>
                ...
                {isOpen && (
                  <div style={{ borderTop: '1px solid var(--border)' }}>
                    {itemsLoading ? (
                      ...
                      const actionLabel = TYPE_LABEL[item.type];
                      return (
                        <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderBottom: index < items.length - 1 ? '1px solid var(--border)' : 'none', background: item.is_completed ? 'var(--forest-pale)' : 'white', transition: 'background 0.2s' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, marginTop: 3, alignItems: 'center' }}>
                              ...
                              {item.is_completed && item.quiz_score != null && <span style={{ fontSize: '0.7rem', color: 'var(--forest)', fontWeight: 700 }}>{t('स्कोर', 'Score')}: {item.quiz_score}%</span>}
                            </div>
                          </div>
                          ...
                          {item.is_completed ? (
                            ...
                            <button className="btn-primary" style={{ padding: '6px 14px', fontSize: '0.75rem', flexShrink: 0 }} disabled={progressMut.isPending} onClick={() => progressMut.mutate({ contentItemId: item.id })}>
                              {actionLabel ? t(actionLabel.hi, actionLabel.en) : t('खोलें', 'Open')}
                            </button>
                          )}
                        </div>
                      );
