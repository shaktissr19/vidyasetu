...
import { CardSkeleton, StatusBadge } from '@/components/ui/index';
...
export default function ExamsPage() {
  ...
  const registerMut = useMutation({
    ...
    onSuccess:  (_, examId) => { toast.success('✅ Registered successfully!'); refetch(); },
    onError:    (err)  => toast.error(err.response?.data?.error?.message || 'Failed to register'),
  });
  ...
  return (
    <div className="animate-fade-up">
      <h1 className="font-display font-extrabold text-2xl mb-5" style={{ color: 'var(--navy)' }}>
        📝 {t('परीक्षाएँ', 'Exams & Competitions')}
      </h1>

      {upcoming.length > 0 && (
        <div className="mb-6">
          <h2 className="font-display font-bold text-base mb-3" style={{ color: 'var(--navy)' }}>
            🔴 {t('आगामी परीक्षाएँ', 'Upcoming Exams')}
          </h2>
          <div className="space-y-3 stagger">
            {upcoming.map((exam) => (
              <div key={exam.id} className="card animate-fade-up"
                style={{ borderLeft: `4px solid ${exam.status === 'LIVE' ? 'var(--forest)' : 'var(--saffron)'}` }}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-display font-bold text-base truncate" style={{ color: 'var(--navy)' }}>{exam.title}</h3>
                      <span className={`badge ${exam.status === 'LIVE' ? 'badge-green' : 'badge-orange'} flex-shrink-0`}>
                        {exam.status === 'LIVE' ? '🟢 LIVE' : '📋 Open'}
                      </span>
                    </div>
                  ...
                  <div className="flex gap-2">
                    {exam.registration_id ? (
                      exam.status === 'LIVE' ? (
                        <button className="btn-primary" style={{ background: 'linear-gradient(135deg, var(--forest), var(--forest-light))' }}
                      ...
                      <button className="btn-primary" disabled={registerMut.isPending}
                        onClick={() => registerMut.mutate(exam.id)}>
                        Register Free
                      </button>
                    )}
                  </div>
              ...
              <div key={exam.id} className="card flex flex-wrap items-center justify-between gap-3 animate-fade-up">
                ...
                {exam.attempt_status === 'EVALUATED' ? (
                  <div className="text-right">
                    <p className="font-display font-extrabold text-xl" style={{ color: 'var(--saffron)' }}>
                      {exam.score}/{exam.total_marks}
                    </p>
                    <button className="text-xs mt-0.5" style={{ color: 'var(--saffron)' }}
                      onClick={() => router.push(`/competition/${exam.id}/leaderboard`)}>
                      View Leaderboard →
                    </button>
                  </div>
                ) : (
