...
export default function ReportCardPage() {
  ...
  const maxTotal = results.reduce(sum => sum + 100, 0);
  ...
  return (
    <div className="animate-fade-up">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <h1 className="font-display font-extrabold text-2xl" style={{ color: 'var(--navy)' }}>📄 {t('रिपोर्ट कार्ड', 'Report Card')}</h1>
        <div className="flex gap-2">
          <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'var(--saffron-pale)' }}>
            {[1, 2, 3].map(n => (
              <button key={n} onClick={() => setTerm(n)} className="px-3 py-1.5 rounded-lg text-sm font-bold transition-all"
                style={{ background: term === n ? 'white' : 'transparent', color: term === n ? 'var(--saffron)' : 'var(--slate)' }}>
                {t(`टर्म ${n}`, `Term ${n}`)}
              </button>
            ))}
          </div>
          <button className="btn-primary" onClick={() => toast('📥 Downloading report card PDF...')}>📥 {t('PDF', 'PDF')}</button>
        </div>
      </div>
      ...
      {isLoading ? <CardSkeleton /> : (
        <div className="card max-w-2xl" style={{ border: '2px solid var(--navy)' }}>
          <div className="text-center pb-4 mb-4" style={{ borderBottom: '2px solid var(--navy)' }}>
            <p className="text-xs font-semibold" style={{ color: 'var(--slate)' }}>{student?.school_name || student?.schoolName || '—'}</p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5 text-sm">
            {[
              ...
              [t('कक्षा / अनुभाग', 'Class / Section'), student?.class_name ? `${student.class_name}-${student.section || ''}` : student?.classLabel || '—'],
              [t('रोल नंबर', 'Roll Number'), String(student?.roll_number || student?.rollNumber || '—')],
              [t('उपस्थिति', 'Attendance'), `${attendance?.pct ?? attendance?.percentage ?? 0}% (${t('अच्छा', 'Good')})`],
            ].map(([label, value]) => (
              <div key={label}>
                <span className="text-xs font-semibold" style={{ color: 'var(--slate)' }}>{label}</span>
                <p className="font-bold mt-0.5" style={{ color: 'var(--navy)' }}>{value || '—'}</p>
              </div>
            ))}
          </div>

          {results.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--slate)' }}>{t('इस टर्म के परिणाम अभी उपलब्ध नहीं हैं', 'Results not available for this term yet')}</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="tbl">
                <thead><tr><th>{t('विषय', 'Subject')}</th><th className="text-right">{t('अंक', 'Marks')}</th><th className="text-right">{t('ग्रेड', 'Grade')}</th><th className="text-right">{t('टिप्पणी', 'Remark')}</th></tr></thead>
                <tbody>
                  {results.map((row, index) => {
                    const score = Number(row.score || 0);
                    const { grade, color } = gradeFromScore(score, 100);
                    return (
                      <tr key={index}>
                        <td className="font-semibold">{row.subject_name || row.exam_name || '—'}</td>
                        <td className="text-right font-bold">{score}/100</td>
                        ...
                        <td className="text-right text-xs" style={{ color: 'var(--slate)' }}>
                          {score >= 90 ? 'Outstanding' : score >= 75 ? 'Very Good' : score >= 60 ? 'Good' : 'Needs work'}
                        </td>
                      </tr>
                    );
                  })}
                  <tr style={{ background: '#F8F9FC', fontWeight: 700 }}>
                    ...
                    <td className="text-right font-extrabold font-display" style={{ color: gradeFromScore(total, maxTotal).color }}>{gradeFromScore(total, maxTotal).grade}</td>
