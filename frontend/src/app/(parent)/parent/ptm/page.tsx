...
export default function ParentPtmPage(){
  ...
  useEffect(()=>{if(typeof window==='undefined')return;const params=new URLSearchParams(window.location.search);const queryStudent=params.get('studentId');const queryIntervention=params.get('interventionId');if(queryStudent)setStudentId(queryStudent);if(queryIntervention)setInterventionId(queryIntervention);},[]);
  ...
  const bookM=useMutation({mutationFn:(slotId:string)=>bookParentPtmSlot(studentId,slotId,{interventionId:interventionId||null}),onSuccess:async()=>{toast.success(t('PTM बुक हो गई','PTM booked'));await refresh();},onError:(e:unknown)=>toast.error(apiErrorText(e,'Could not book PTM'))});
  ...
  const error=childrenQ.error||optionsQ.error||bookingsQ.error;
  ...
  return <div className="animate-fade-up space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="font-display font-extrabold text-2xl" style={{color:'var(--navy)'}}>🤝 {t('अभिभावक–शिक्षक बैठक','Parent–Teacher Meetings')}</h1><p className="text-sm mt-1" style={{color:'var(--slate)'}}>{t('सीखने के प्रमाण और शिक्षक हस्तक्षेप के आधार पर सही शिक्षक के साथ फॉलो-अप बुक करें।','Book follow-up with the right Teacher using learning evidence and intervention context.')}</p></div><select className="input select w-auto min-w-[240px]" value={studentId} onChange={e=>{setStudentId(e.target.value);setInterventionId('');}}>{(childrenQ.data||[]).map(child=><option key={child.id} value={child.id}>{child.name} · Class {child.class_name}{child.section?`-${child.section}`:''}</option>)}</select></div>
    ...
    <div className="card"><h2 className="font-bold text-lg mb-3">{t('उपलब्ध शिक्षक स्लॉट','Available Teacher slots')}</h2>{optionsQ.isLoading?<p>Loading…</p>:!(optionsQ.data||[]).length?<p className="text-sm" style={{color:'var(--slate)'}}>{t('अभी कोई बुक करने योग्य स्लॉट नहीं है।','No bookable PTM slots are currently open.')}</p>:<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">{(optionsQ.data||[]).map(s=><div key={s.id} className="p-4 rounded-xl" style={{border:'1px solid var(--border)'}}><div className="font-bold">{s.teacher_name||'Teacher'}</div>{s.subjects&&<div className="text-xs mt-1" style={{color:'var(--slate)'}}>{s.subjects}</div>}<div className="text-sm mt-3">{local(s.starts_at)}</div><div className="text-sm">{s.location||'School campus'}</div><button className="btn btn-primary mt-3 text-sm" disabled={bookM.isPending} onClick={()=>bookM.mutate(s.id)}>{t('यह स्लॉट बुक करें','Book this slot')}</button></div>)}</div>}</div>
