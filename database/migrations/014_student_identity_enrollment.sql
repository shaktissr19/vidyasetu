...
-- Legacy accounts receive a guaranteed-unique migration username. New
...
-- Only the FIRST introduction of school_link_status may infer that legacy
-- Students already attached to a School are approved. A later migration rerun
-- must never promote a PENDING/REJECTED self-registration.
DO $$
DECLARE
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'school_link_status'
  ) INTO status_column_exists;

  IF NOT status_column_exists THEN
    ALTER TABLE students
      ADD COLUMN school_link_status student_school_link_status NOT NULL DEFAULT 'APPROVED';

    UPDATE students
    SET school_link_status = CASE
      WHEN school_id IS NOT NULL THEN 'APPROVED'::student_school_link_status
...
UPDATE students SET grade_level = '8' WHERE grade_level IS NULL;
