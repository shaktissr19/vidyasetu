...
async function getApprovedStudents(schoolId, paginationQuery, filters = {}) {
  ...
  const [{ rows }, { rows: [countRow] }] = await Promise.all([
    ...
    query(`SELECT COUNT(*) FROM students st JOIN users u ON u.id = st.user_id WHERE ${where}`.replace('sT.student_code', 'st.student_code'), params),
  ]);

  return { students: rows, meta: paginationMeta(parseInt(countRow.count, 10), page, limit) };
}

async function getRosterCounts(schoolId) {
...
module.exports = { getApprovedStudents, getRosterCounts };
