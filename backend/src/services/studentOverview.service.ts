...
export interface StudentOverviewRow extends QueryResultRow {
  id: UUID;
  user_id: UUID;
  student_code: string;
  grade_level: string;
...
interface AcademicRankRow extends QueryResultRow {
  rank: string | number;
...
interface RecentResultRow extends QueryResultRow {
  id: UUID;
  title: string;
  type: string;
  start_time: string | Date;
...
interface TimetableRow extends QueryResultRow {
  id: UUID;
  day: string;
...
interface RowsOnly<T extends QueryResultRow> {
  rows: T[];
}

function emptyRows<T extends QueryResultRow>(): RowsOnly<T> {
  return { rows: [] };
}

export async function getStudent(userId: UUID): Promise<StudentOverviewRow> {
  const { rows: [student] } = await query<StudentOverviewRow>(
...
async function getAcademicRank(
  student: StudentOverviewRow,
  scope: 'class' | 'school',
): Promise<{ rank: number; average: number } | null> {
  if (student.school_link_status !== 'APPROVED' || !student.class_id || !student.school_id) {
    return null;
  }
  ...
  const { rows: [row] } = await query<AcademicRankRow>(
...
export async function getDashboard(userId: UUID): Promise<{
  student: {
    id: UUID;
    userId: UUID;
    studentCode: string;
  ...
  schoolLink: enrollmentService.StudentLinkSummary | null;
  monthlyAttendance: AttendanceSummaryRow | null;
  ...
  const approvedSchool = Boolean(
    student.school_link_status === 'APPROVED' && student.school_id && student.class_id,
  );
  ...
  const [
    ...
    approvedSchool
      ...
      : Promise.resolve(emptyRows<AttendanceSummaryRow>()),
    ...
    approvedSchool
      ...
      : Promise.resolve(emptyRows<TimetableRow>()),
    approvedSchool
      ...
      : Promise.resolve(emptyRows<AnnouncementRow>()),
  ...
  const academicAverage = schoolTests.length
    ? Number(
        (
          schoolTests.reduce((sum, row) => sum + Number(row.percentage || 0), 0)
          / schoolTests.length
        ).toFixed(1),
      )
    : null;
