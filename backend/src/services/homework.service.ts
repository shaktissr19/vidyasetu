...
interface ActorRow extends QueryResultRow { teacher_id: UUID | null; }
...
async function assertClassSubjectAccess(
  ...
  const { rows: [subject] } = await query<IdRow>('SELECT id FROM subjects WHERE code=$1 LIMIT 1', [subjectCode]);
  ...
  if (!assignment) throw appError('Teachers can manage homework only for their assigned class and subject', 403);
}
...
async function getStaffAssignment(
  ...
  const { rows: [assignment] } = await query<AssignmentAccessRow>(
    `SELECT id,school_id,class_id,subject_code,title,due_at,max_marks,status,created_by
     FROM homework_assignments WHERE id=$1 AND school_id=$2`,
    [homeworkId, schoolId],
  );
...
export async function submitStudentHomework(userId: UUID, homeworkId: UUID, input: HomeworkSubmissionInput) {
  const student = await getStudentContext(userId);
  ...
  const { rows: [assignment] } = await query<AssignmentAccessRow>(
    `SELECT id,school_id,class_id,subject_code,title,due_at,max_marks,status,created_by
     FROM homework_assignments WHERE id=$1 AND school_id=$2 AND class_id=$3`,
  ...
  const { rows: [submission] } = await query(
    `INSERT INTO homework_submissions
     ...
     ON CONFLICT (homework_id,student_id) DO UPDATE SET
       answer_text=EXCLUDED.answer_text,attachment_url=EXCLUDED.attachment_url,
       status=EXCLUDED.status,submitted_at=NOW(),marks_awarded=NULL,feedback=NULL,
...
export async function listSchoolHomework(
  ...
  if (role === 'TEACHER') {
    ...
    conditions.push(`EXISTS (
      ...
      WHERE ta.teacher_id=$${params.length} AND ta.school_id=ha.school_id
        AND ta.class_id=ha.class_id AND ta.subject_code=ha.subject_code
    )`);
  }

  const { rows } = await query(
    `SELECT ha.id,ha.class_id,ha.title,ha.description,ha.instructions,ha.attachment_url,
            ha.subject_code,sub.name AS subject_name,ha.due_at,ha.max_marks,ha.status,
            ha.published_at,ha.closed_at,ha.created_at,sc.class_name,sc.section,
            u.name AS created_by_name,
            (SELECT COUNT(*)::int FROM homework_submissions hs WHERE hs.homework_id=ha.id) AS submitted_count,
            (SELECT COUNT(*)::int FROM homework_submissions hs
             WHERE hs.homework_id=ha.id AND hs.status IN ('REVIEWED','RETURNED')) AS reviewed_count,
            (SELECT COUNT(*)::int FROM students st
             WHERE st.school_id=ha.school_id AND st.class_id=ha.class_id
               AND st.school_link_status='APPROVED' AND st.status='ACTIVE') AS class_student_count
     FROM homework_assignments ha
     JOIN school_classes sc ON sc.id=ha.class_id
     LEFT JOIN subjects sub ON sub.code=ha.subject_code
...
export async function createHomework(
  ...
  const { rows: [created] } = await query(
    ...
    [schoolId,input.classId,subjectCode,title,description,cleanOptional(input.instructions),
     cleanOptional(input.attachmentUrl),dueAt.toISOString(),maxMarks,userId],
  );
  return created;
}
...
export async function updateHomework(
  ...
  const { rows: [updated] } = await query(
    `UPDATE homework_assignments SET
       class_id=$1,subject_code=$2,title=COALESCE($3,title),description=COALESCE($4,description),
       ...
       due_at=$9,max_marks=$10,updated_at=NOW()
     WHERE id=$11 AND school_id=$12 RETURNING *`,
    [classId,subjectCode,input.title === undefined ? null : cleanRequired(input.title, 'Title'),
     input.description === undefined ? null : cleanRequired(input.description, 'Description'),
     input.instructions !== undefined,cleanOptional(input.instructions),input.attachmentUrl !== undefined,
     cleanOptional(input.attachmentUrl),dueAt.toISOString(),maxMarks,homeworkId,schoolId],
  );
  return updated;
}
...
async function notifyClassHomework(assignment: AssignmentAccessRow): Promise<void> {
  ...
  const results = await Promise.allSettled(students.map((student) => saveNotification({
...
export async function publishHomework(schoolId: UUID, userId: UUID, role: UserRole, homeworkId: UUID) {
  ...
  const { rows: [published] } = await query<AssignmentAccessRow>(
    `UPDATE homework_assignments SET status='PUBLISHED',published_at=NOW(),updated_at=NOW()
...
export async function closeHomework(schoolId: UUID, userId: UUID, role: UserRole, homeworkId: UUID) {
  const assignment = await getStaffAssignment(schoolId, userId, role, homeworkId);
  ...
  const { rows: [closed] } = await query(
    `UPDATE homework_assignments SET status='CLOSED',closed_at=NOW(),updated_at=NOW()
...
export async function reviewHomeworkSubmission(
  ...
  const { rows: [submission] } = await query<SubmissionAccessRow>(
    `SELECT hs.id,hs.homework_id,hs.student_id,st.user_id AS student_user_id,hs.status
     ...
     WHERE hs.id=$1 AND hs.homework_id=$2 AND st.school_id=$3 AND st.class_id=$4`,
  ...
  if (nextStatus === 'RETURNED' && !feedback) {
    throw appError('Feedback is required when returning work for revision', 400);
  }
  const { rows: [updated] } = await query(
