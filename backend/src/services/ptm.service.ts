...
export interface CreatePtmBookingInput {
  parentNote?: string | null;
  interventionId?: UUID | null;
}
export interface PtmOutcomeInput {
...
interface BookingRow extends QueryResultRow {
  ...
  intervention_id: UUID | null; status: PtmBookingStatus; parent_note: string | null; outcome_note: string | null;
  agreed_action: string | null; follow_up_at: string | Date | null; booked_at: string | Date;
  ...
  intervention_title?: string | null; intervention_status?: string | null;
}
...
interface InterventionLinkRow extends QueryResultRow { id: UUID; teacher_id: UUID; school_id: UUID; status: string; }

function httpError(message: string, statusCode: number): Error & { statusCode: number } {
  return Object.assign(new Error(message), { statusCode });
}
function clean(value: string | null | undefined): string | null {
  const text = value?.trim(); return text ? text : null;
}
...
async function teacherAssignedToClass(client: PoolClient, schoolId: UUID, teacherId: UUID, classId: UUID): Promise<void> {
  const { rows: [row] } = await client.query<IdRow>(
    `SELECT ta.id FROM teacher_assignments ta WHERE ta.school_id=$1 AND ta.teacher_id=$2 AND ta.class_id=$3 LIMIT 1`,
...
function sessionTimes(input: CreatePtmSessionInput) {
  const starts = asDate(input.startsAt, 'PTM start'); const ends = asDate(input.endsAt, 'PTM end');
  const opens = asDate(input.bookingOpensAt, 'Booking open time'); const closes = asDate(input.bookingClosesAt, 'Booking close time');
...
export async function createSession(schoolId: UUID, actorId: UUID, role: UserRole, input: CreatePtmSessionInput): Promise<SessionRow> {
  await requireSchema(); requireAdmin(role); const { starts, ends, opens, closes } = sessionTimes(input);
  return transaction(async (client) => {
    ...
    if (!session) throw new Error('PTM session insert returned no row'); return session;
  });
}
...
export async function changeSessionStatus(schoolId: UUID, role: UserRole, sessionId: UUID, input: PtmSessionStatusInput): Promise<SessionRow> {
  ...
  return transaction(async (client) => {
    ...
    if (input.status==='CANCELLED') {
      if (current.calendar_event_id) await client.query('UPDATE school_calendar_events SET is_active=FALSE WHERE id=$1 AND school_id=$2',[current.calendar_event_id,schoolId]);
    ...
    if (!updated) throw new Error('PTM status update returned no row'); return updated;
  });
}
export async function listSchoolSessions(schoolId: UUID): Promise<SessionRow[]> {
  await requireSchema();
  return (await query<SessionRow>(
    `SELECT s.*,COUNT(sl.id)::int AS slot_count,COUNT(b.id) FILTER(WHERE b.status='BOOKED')::int AS booked_count
     FROM ptm_sessions s LEFT JOIN ptm_slots sl ON sl.session_id=s.id AND sl.is_active=TRUE
     LEFT JOIN ptm_bookings b ON b.slot_id=sl.id WHERE s.school_id=$1 GROUP BY s.id ORDER BY s.starts_at DESC`,[schoolId],
  )).rows;
}
export async function createSlot(schoolId: UUID, actorId: UUID, role: UserRole, sessionId: UUID, input: CreatePtmSlotInput): Promise<SlotRow> {
  await requireSchema(); const teacher=await activeTeacher(schoolId,input.teacherId);
  if (role==='TEACHER') { const self=await teacherForUser(schoolId,actorId); if (self.id!==teacher.id) throw httpError('Teachers may create slots only for themselves',403); }
  else requireAdmin(role);
  const starts=asDate(input.startsAt,'Slot start'); const ends=asDate(input.endsAt,'Slot end');
  ...
  try {
    ...
    if (!row) throw new Error('PTM slot insert returned no row'); return row;
  } catch (error: unknown) {
    if ((error as { code?: string })?.code==='23505') throw httpError('This Teacher already has a PTM slot at that start time',409); throw error;
  }
}
function bookingSelect(where: string): string {
  return `SELECT b.*,ps.title AS session_title,sl.starts_at,sl.ends_at,sl.location,
    ...
    li.title AS intervention_title,li.status AS intervention_status
    ...
    JOIN users pu ON pu.id=b.parent_user_id LEFT JOIN learning_interventions li ON li.id=b.intervention_id WHERE ${where}`;
}
export async function listSchoolBookings(schoolId: UUID,actorId: UUID,role: UserRole,sessionId?: UUID): Promise<BookingRow[]> {
  await requireSchema(); const params: unknown[]=[schoolId]; let where='b.school_id=$1';
  if (role==='TEACHER') { const teacher=await teacherForUser(schoolId,actorId); params.push(teacher.id); where+=` AND b.teacher_id=$${params.length}`; } else requireAdmin(role);
...
export async function listSchoolSlots(schoolId: UUID,actorId: UUID,role: UserRole,sessionId?: UUID): Promise<SlotRow[]> {
  await requireSchema(); const params: unknown[]=[schoolId]; let where='sl.school_id=$1';
  if (role==='TEACHER') { const teacher=await teacherForUser(schoolId,actorId); params.push(teacher.id); where+=` AND sl.teacher_id=$${params.length}`; } else requireAdmin(role);
...
async function validateInterventionForBooking(client: PoolClient,student: StudentRow,teacherId: UUID,interventionId?: UUID|null): Promise<void> {
  if (!interventionId) return;
  const { rows:[link] }=await client.query<InterventionLinkRow>(
    `SELECT li.id,li.teacher_id,li.school_id,li.status
     FROM learning_interventions li JOIN learning_intervention_students lis ON lis.intervention_id=li.id
     WHERE li.id=$1 AND lis.student_id=$2 AND lis.status<>'REMOVED' FOR UPDATE OF li`,[interventionId,student.id],
  );
  if (!link || link.school_id!==student.school_id) throw httpError('This learning intervention is not available for the Student',403);
  if (link.teacher_id!==teacherId) throw httpError('Choose a PTM slot with the Teacher responsible for this intervention',400);
  if (['RESOLVED','CLOSED'].includes(link.status)) throw httpError('A resolved learning intervention cannot be escalated to PTM',409);
  await client.query(`UPDATE learning_interventions SET status='PTM_REQUESTED' WHERE id=$1 AND status IN ('OPEN','PARENT_ACKNOWLEDGED','IN_PROGRESS')`,[interventionId]);
}
export async function bookParentSlot(parentUserId: UUID,studentId: UUID,slotId: UUID,input: CreatePtmBookingInput): Promise<BookingRow> {
  ...
  const booking=await transaction(async(client)=>{
    ...
    await validateInterventionForBooking(client,student,slot.teacher_id,input.interventionId);
    try {
      const { rows:[created] }=await client.query<BookingRow>(
        `INSERT INTO ptm_bookings(session_id,slot_id,school_id,teacher_id,student_id,parent_user_id,intervention_id,parent_note)
         VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[slot.session_id,slot.id,student.school_id,slot.teacher_id,student.id,parentUserId,input.interventionId||null,clean(input.parentNote)],
      ); if (!created) throw new Error('PTM booking insert returned no row'); return created;
    } catch(error:unknown) { if ((error as {code?:string})?.code==='23505') throw httpError('This PTM slot or Teacher meeting is already booked',409); throw error; }
  });
  const { rows:[full] }=await query<BookingRow>(`${bookingSelect('b.id=$1')} LIMIT 1`,[booking.id]); if (!full) throw new Error('PTM booking could not be read back');
  ...
  await Promise.all([...recipients].map((recipientId)=>saveNotification({userId:recipientId,schoolId:student.school_id,type:'PTM_BOOKED',title:`PTM booked · ${student.name}`,body:`${full.teacher_name||'Teacher'} · ${new Date(full.starts_at as string).toLocaleString('en-IN')}.`,refId:full.id,refType:'PTM_BOOKING'})));
  return full;
}
export async function cancelParentBooking(parentUserId: UUID,bookingId: UUID): Promise<BookingRow> {
  ...
  const updated=await transaction(async(client)=>{
    ...
    if(!row) throw httpError('PTM booking not found',404); if(row.parent_user_id!==parentUserId) throw httpError('Only the Parent who booked this appointment can cancel it',403); if(row.status!=='BOOKED') throw httpError('Only a booked PTM appointment can be cancelled',409);
    const { rows:[slot] }=await client.query<SlotRow>('SELECT * FROM ptm_slots WHERE id=$1',[row.slot_id]); if(slot&&new Date(slot.starts_at)<=new Date()) throw httpError('A PTM appointment cannot be cancelled after it starts',409);
    const { rows:[next] }=await client.query<BookingRow>(`UPDATE ptm_bookings SET status='CANCELLED',cancelled_at=NOW() WHERE id=$1 RETURNING *`,[bookingId]); if(!next) throw new Error('PTM cancellation returned no row'); return next;
  });
  const { rows:[full] }=await query<BookingRow>(`${bookingSelect('b.id=$1')} LIMIT 1`,[updated.id]); if(!full) throw new Error('Cancelled PTM booking could not be read back');
  ...
  await Promise.all([...recipients].map((recipientId)=>saveNotification({userId:recipientId,schoolId:full.school_id,type:'PTM_CANCELLED',title:'PTM appointment cancelled',body:`${full.student_name} · ${full.teacher_name}`,refId:full.id,refType:'PTM_BOOKING'})));
  return full;
}
export async function updateOutcome(schoolId: UUID,actorId: UUID,role: UserRole,bookingId: UUID,input: PtmOutcomeInput): Promise<BookingRow> {
  await requireSchema();
  const followUp=input.followUpAt?asDate(input.followUpAt,'Follow-up time'):null;
  const updated=await transaction(async(client)=>{
    const { rows:[row] }=await client.query<BookingRow & {slot_starts_at:string|Date}>(
      `SELECT b.*,sl.starts_at AS slot_starts_at FROM ptm_bookings b JOIN ptm_slots sl ON sl.id=b.slot_id WHERE b.id=$1 AND b.school_id=$2 FOR UPDATE OF b`,[bookingId,schoolId],
    );
    if(!row) throw httpError('PTM booking not found in this School',404); if(row.status!=='BOOKED') throw httpError('Only a booked appointment can be completed',409);
    if(role==='TEACHER'){const teacher=await teacherForUser(schoolId,actorId);if(teacher.id!==row.teacher_id)throw httpError('Teachers may update only their own PTM appointments',403);}else requireAdmin(role);
    ...
    const { rows:[next] }=await client.query<BookingRow>(
      `UPDATE ptm_bookings SET status=$2,outcome_note=$3,agreed_action=$4,follow_up_at=$5,completed_at=NOW() WHERE id=$1 RETURNING *`,
      [bookingId,input.status,clean(input.outcomeNote),clean(input.agreedAction),followUp?.toISOString()||null],
    ); if(!next) throw new Error('PTM outcome update returned no row');
    if(row.intervention_id&&input.status==='COMPLETED') {
      await client.query(
        `UPDATE learning_interventions SET status=CASE WHEN status='PTM_REQUESTED' THEN 'IN_PROGRESS' ELSE status END,
           outcome_note=COALESCE($2,outcome_note) WHERE id=$1`,[row.intervention_id,clean(input.outcomeNote)],
      );
      await client.query(
        `UPDATE learning_intervention_students SET status=CASE WHEN status IN ('ASSIGNED','ACKNOWLEDGED') THEN 'IN_PROGRESS' ELSE status END
         WHERE intervention_id=$1 AND student_id=$2 AND status<>'REMOVED'`,[row.intervention_id,row.student_id],
      );
    }
    return next;
  });
  const { rows:[full] }=await query<BookingRow>(`${bookingSelect('b.id=$1')} LIMIT 1`,[updated.id]); if(!full) throw new Error('Updated PTM booking could not be read back');
  await Promise.all((await familyRecipients(full.student_id)).map((recipientId)=>saveNotification({userId:recipientId,schoolId,type:'PTM_UPDATED',title:`PTM ${input.status==='COMPLETED'?'completed':'marked no-show'}`,body:`${full.teacher_name||'Teacher'} · ${full.student_name}`,refId:full.id,refType:'PTM_BOOKING'})));
  return full;
}
export async function listParentBookings(parentUserId: UUID,studentId: UUID): Promise<BookingRow[]> {
  await requireSchema(); await parentChild(parentUserId,studentId); return (await query<BookingRow>(`${bookingSelect('b.student_id=$1')} ORDER BY sl.starts_at DESC`,[studentId])).rows;
}
export async function listStudentBookings(userId: UUID): Promise<BookingRow[]> {
  await requireSchema(); const student=await studentForUser(userId); return (await query<BookingRow>(`${bookingSelect('b.student_id=$1')} ORDER BY sl.starts_at DESC`,[student.id])).rows;
}
