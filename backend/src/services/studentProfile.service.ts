...
interface SetupOptionRow extends QueryResultRow {
  school_id: UUID;
  school_name: string;
  city: string | null;
...
export interface StudentSetupClass {
  id: UUID;
  className: string;
  section: string;
  academicYear: string;
...
export interface StudentSetupSchool {
  id: UUID;
  name: string;
  city: string | null;
  district: string | null;
...
export interface CompleteStudentProfileInput {
  name: string;
  language: LanguageCode;
...
interface SelectedClassRow extends QueryResultRow {
  id: UUID;
  school_id: UUID;
  class_name: string;
  section: string;
...
async function findExistingStudent(client: PoolClient, userId: UUID): Promise<IdRow | undefined> {
...
export async function getSetupOptions(): Promise<{
  schools: StudentSetupSchool[];
  gradeLevels: string[];
}> {
  const { rows } = await query<SetupOptionRow>(
...
export async function getProfileByUserId(userId: UUID): Promise<StudentProfile | null> {
...
export async function getProfileStatus(userId: UUID): Promise<{
  complete: boolean;
...
export async function completeProfile(
  userId: UUID,
  payload: CompleteStudentProfileInput,
  ...
  await transaction(async (client) => {
    const { rows: [user] } = await client.query<LockedUserRow>(
    ...
    const existing = await findExistingStudent(client, userId);
    if (existing) return;

    let classRow: SelectedClassRow | null = null;
    if (schoolId) {
      if (!classId) throw badRequest('Select a class/section when requesting a school');
      const { rows } = await client.query<SelectedClassRow>(
    ...
    const { rows: [student] } = await client.query<IdRow>(
    ...
    if (!student) throw new Error('Student profile insert did not return a row');
    ...
    if (parentMobile || parentEmail) {
      const { rows: [parent] } = await client.query<IdRow>(
      ...
      if (parent) {
        ...
        await client.query(
          ...
          [
            ...
            parentName || null,
            parentMobile || null,
            parentEmail ? String(parentEmail).toLowerCase() : null,
            parentRelation,
          ],
        );
      }
