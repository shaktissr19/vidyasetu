...
export async function chat(
  ...
  if (provider === 'mock' || process.env.NODE_ENV === 'test') return mockResponse(message);
  ...
  await logAIInteraction(studentId, message, responseText).catch(() => undefined);
...
Explain clearly, step by step, in the same language as the student's question.`;
  ...
  let answerId: UUID;
  if (existing) {
    await query(`UPDATE doubt_answers SET body = $1, updated_at = NOW() WHERE id = $2`, [response, existing.id]);
...
async function logAIInteraction(studentId: UUID | null, question: string, answer: string): Promise<void> {
  logger.info(`AI interaction — Student: ${studentId || 'n/a'} | Q: ${String(question).substring(0, 60)} | A: ${String(answer).substring(0, 60)}`);
}

function mockResponse(message: string): { response: string } {
