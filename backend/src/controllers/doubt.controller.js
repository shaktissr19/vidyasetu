...
async function getUserSchoolContext(req) {
  if (req.user.role === 'STUDENT') {
    const { rows: [s] } = await query(
      `SELECT s.id AS student_id, s.school_id, s.class_id, sc.class_name, sc.section
       FROM students s
  ...
  if (req.user.role === 'SCHOOL_ADMIN') {
    return { school_id: req.user.schoolId || null };
  }
...
async function list(req, res, next) {
  try {
    ...
    if (subjectCode) {
      conditions.push(`d.subject_code = $${i++}`);
    ...
    if (mine === 'true' && ctx?.student_id) {
      conditions.push(`d.student_id = $${i++}`);
      params.push(ctx.student_id);
    }
    ...
    const [{ rows }, { rows: [countRow] }] = await Promise.all([
      query(
        `SELECT d.id, d.title, d.body, d.image_url, d.status,
         ...
         WHERE ${where}
         ORDER BY
           CASE d.status WHEN 'OPEN' THEN 1 WHEN 'RESOLVED' THEN 2 ELSE 3 END,
           d.created_at DESC
...
async function create(req, res, next) {
  try {
    ...
    const { rows: [doubt] } = await query(
      `INSERT INTO doubts
         (student_id, school_id, subject_code, chapter_id, content_item_id,
          title, body, image_url)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
       RETURNING *`,
      [ctx.student_id, ctx.school_id, subjectCode, chapterId || null, contentItemId || null,
...
async function get(req, res, next) {
  try {
    ...
    if (ctx?.school_id && doubt.school_id !== ctx.school_id) return R.forbidden(res, 'Doubt is outside your school');
...
async function answer(req, res, next) {
  try {
    const ctx = await getUserSchoolContext(req);
    const { rows: [doubt] } = await query(
      `SELECT id, student_id, school_id, status FROM doubts WHERE id = $1`,
      [req.params.doubtId]
    );
    if (!doubt) return R.notFound(res, 'Doubt not found');
    if (ctx?.school_id && doubt.school_id !== ctx.school_id) return R.forbidden(res, 'Doubt is outside your school');
    ...
    if (req.user.role === 'STUDENT' && ctx?.student_id) {
      const { rows: [existingXP] } = await query(
        `SELECT id FROM xp_events
         WHERE student_id = $1 AND event_type = 'DOUBT_ANSWERED' AND reference_id = $2
         LIMIT 1`,
        [ctx.student_id, answer.id]
      );
      if (!existingXP) {
  ...
  try {
    ...
    const { rows: [answer] } = await query(
      `SELECT da.id, da.author_id, d.school_id
       FROM doubt_answers da
       JOIN doubts d ON d.id = da.doubt_id
    ...
    const ctx = await getUserSchoolContext(req);
    if (ctx?.school_id && answer.school_id !== ctx.school_id) return R.forbidden(res, 'Answer is outside your school');
    ...
    const { rows: [authorStudent] } = await query(
    ...
    if (authorStudent) {
      const { rows: [existingXP] } = await query(
        `SELECT id FROM xp_events
         WHERE student_id = $1 AND event_type = 'DOUBT_UPVOTED' AND reference_id = $2
         LIMIT 1`,
        [authorStudent.id, answerId]
      );
      if (!existingXP) {
        await studentService.awardXP(authorStudent.id, 'DOUBT_UPVOTED', 5, answerId, 'DOUBT_ANSWER', 'Peer upvoted your answer');
      }
    }
    ...
    if (bestAnswerId) {
      ...
      await query(
        `UPDATE doubt_answers
         SET is_accepted = (id = $1), updated_at = NOW()
         WHERE doubt_id = $2`,
        [bestAnswerId, req.params.doubtId]
      );
    }
...
async function aiAnswer(req, res, next) {
  try {
    ...
    const { rows: [doubt] } = await query(
      `SELECT student_id FROM doubts WHERE id = $1`,
      [req.params.doubtId]
    );
    if (!doubt) return R.notFound(res, 'Doubt not found');
