...
export async function getProfileSetupOptions(
  ...
  try {
    return R.ok(res, await studentProfileService.getSetupOptions());
  } catch (err: unknown) {
    next(err);
  }
}
...
export async function getSchoolLink(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<Response | void> {
...
export async function markContentComplete(
  ...
  try {
    ...
    return R.ok(
      res,
      await studentLearningService.markContentComplete(user.userId, req.params.contentItemId),
    );
  } catch (err: unknown) {
    next(err);
  }
...
export async function removeOfflineDownload(
  ...
  try {
    ...
    return R.ok(
      res,
      await studentPortalService.removeOfflineDownload(user.userId, req.params.contentItemId),
    );
  } catch (err: unknown) {
    next(err);
  }
