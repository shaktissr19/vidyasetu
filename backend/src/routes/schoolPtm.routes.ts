...
const sessionSchema=z.object({title:z.string().trim().min(3).max(220),description:z.string().trim().max(3000).optional().nullable(),startsAt:z.string().datetime(),endsAt:z.string().datetime(),bookingOpensAt:z.string().datetime(),bookingClosesAt:z.string().datetime()});
...
const outcomeSchema=z.object({status:z.enum(['COMPLETED','NO_SHOW']),outcomeNote:z.string().trim().max(1600).optional().nullable(),agreedAction:z.string().trim().max(1600).optional().nullable(),followUpAt:z.string().datetime().optional().nullable()});
