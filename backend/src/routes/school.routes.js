...
}).refine(v => !!v.mobile || !!v.email, { message: 'Parent mobile or email is required' });
...
const questionSchema = z.object({
  questionText: z.string().min(3), questionHi: z.string().optional(),
  optionA: z.string().min(1), optionB: z.string().min(1), optionC: z.string().min(1), optionD: z.string().min(1),
  correctOption: z.enum(['A','B','C','D']), explanation: z.string().optional(),
  subjectCode: z.string().max(20).optional(), difficulty: z.enum(['EASY','MEDIUM','HARD']).optional(),
});
const examSchema = z.object({
  ...
  startTime: z.string(), endTime: z.string(), resultsAt: z.string().optional(),
  durationMins: z.number().int().positive().max(300).optional(),
  totalQuestions: z.number().int().positive().max(200).optional(),
  marksPerQuestion: z.number().positive().optional(), negativeMarks: z.number().min(0).optional(),
  registrationStart: z.string().optional(), registrationEnd: z.string().optional(), instructions: z.string().optional(),
  questions: z.array(questionSchema).max(200).optional(),
});
const statusSchema = z.object({ status: z.enum(['DRAFT','REGISTRATION_OPEN','REGISTRATION_CLOSED','LIVE','SCORING','COMPLETED','CANCELLED']) });
...
// Readable by School Admin, Super Admin and Teachers.
...
// Administration-only operational writes.
