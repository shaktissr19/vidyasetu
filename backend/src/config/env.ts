...
type Environment = z.infer<typeof schema> | NodeJS.ProcessEnv;
...
if (!parsed.success) {
  ...
  // Preserve the current development behavior: warn, but only fail hard in production.
  if (process.env.NODE_ENV === 'production') process.exit(1);
}

const env: Environment = parsed.success ? parsed.data : process.env;
