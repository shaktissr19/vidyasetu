...
const PACK_DIR = path.resolve(
  __dirname,
  '../../../content/class-8/science/force-and-pressure/pressure-in-liquids',
);

const RESOURCE_SLUG = 'class-8-science-pressure-in-liquids-v1';
const PACK_ID = 'VS-C8-SCI-FP-LIQUID-PRESSURE-V1';
...
function validatePack(manifest: PackManifest, bank: QuestionBank): void {
  if (manifest.packId !== PACK_ID) throw new Error(`Unexpected Pressure in Liquids packId: ${manifest.packId}`);
  ...
  if (manifest.sourceCode !== 'VIDYASETU_ORIGINAL' || bank.sourceCode !== 'VIDYASETU_ORIGINAL') {
    throw new Error('Pressure in Liquids installer only accepts VIDYASETU_ORIGINAL content');
  }
  if (manifest.licence !== 'VIDYASETU_ORIGINAL' || bank.licence !== 'VIDYASETU_ORIGINAL') {
    throw new Error('Pressure in Liquids installer only accepts VIDYASETU_ORIGINAL licence');
  }
  ...
  if (!manifest.languages.includes('en') || !manifest.languages.includes('hi')) {
    throw new Error('Pressure in Liquids pack must declare English and Hindi');
  }
  if (manifest.gradeCodes.length !== 1 || manifest.gradeCodes[0] !== 'CLASS_8') {
    throw new Error('Pressure in Liquids v1 is locked to CLASS_8');
  }
  if (manifest.boardCodes.length !== 1 || manifest.boardCodes[0] !== 'COMMON') {
    throw new Error('Pressure in Liquids v1 is locked to COMMON cross-board scope');
  }
  if (bank.questions.length !== 12) {
    throw new Error(`Pressure in Liquids v1 requires exactly 12 questions; found ${bank.questions.length}`);
  }
...
async function requireSuperAdmin(userId: UUID): Promise<void> {
  ...
  if (user.role !== 'SUPER_ADMIN') {
    throw new Error(`Pressure in Liquids installation requires SUPER_ADMIN; received ${user.role}`);
  }
}
...
async function ensureDraftResource(
  ...
  if (existing) {
    ...
    const created = await createLearningResource(
      {
        title: 'Pressure in liquids made simple',
        titleHi: 'द्रवों में दाब आसान तरीके से',
        summary: 'See how liquids press on container surfaces, investigate why deeper points experience greater pressure, and connect the idea to dams, tanks and diving.',
        summaryHi: 'समझें कि द्रव कंटेनर की सतहों पर दाब कैसे डालते हैं, अधिक गहराई पर दाब क्यों बढ़ता है और इस विचार को बाँध, टंकी व गोताखोरी से जोड़ें।',
        bodyMarkdown: bodyEn,
        ...
        attributionText: 'VidyaSetu Original — Class 8 Science Pressure in Liquids learning pack',
        isOfflineReady: true,
...
function assessmentSlug(assetId: string): string {
  if (assetId === 'VS-LP-PRACTICE-01') return 'class-8-science-pressure-in-liquids-practice-v1';
  if (assetId === 'VS-LP-MASTERY-01') return 'class-8-science-pressure-in-liquids-mastery-v1';
  return `class-8-science-pressure-in-liquids-${assetId.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
}
...
async function ensureDraftAssessments(
  ...
  for (const item of manifest.sequence.filter((entry) => entry.type === 'QUIZ')) {
    ...
    if (existing) {
      ...
      const isMastery = item.assetId === 'VS-LP-MASTERY-01';
      const created = await createAssessment(
        {
          ...
          summary: isMastery
            ? 'A concept-level mastery check covering liquid pressure, depth, experimental controls and real-world application.'
            : 'Low-stakes practice to check how liquids exert pressure and how pressure changes with depth.',
    ...
    await query(
      ...
      [
        assessmentId,
        item.assetId === 'VS-LP-MASTERY-01'
          ? 'द्रव दाब, गहराई, प्रयोग के नियंत्रित कारक और वास्तविक जीवन के अनुप्रयोग की अवधारणा-स्तर की महारत जाँच।'
          : 'द्रव सतहों पर दाब कैसे डालते हैं और गहराई के साथ दाब कैसे बदलता है, इसकी कम-दबाव वाली अभ्यास जाँच।',
      ],
    );
...
async function main(): Promise<void> {
  ...
  console.log(`Pressure in Liquids pack validated: ${manifest.packId} (${manifest.version})`);
  ...
  console.log('PRESSURE IN LIQUIDS PACK STAGED SUCCESSFULLY — DRAFT ONLY');
...
main()
  .catch((error: unknown) => {
    ...
    console.error(`PRESSURE IN LIQUIDS PACK INSTALL FAILED: ${message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
    await pool.end();
