...
async function main(){
  ...
  let { rows:[teacher] }=await query<TeacherRow>(
  ...
  // Move only the disposable slot clock into the past so the outcome rule is
  // tested without weakening the production service's "not before start" guard.
...
main().catch((error:unknown)=>{console.error(`LEARNING SUPPORT 2.0 CERTIFICATION FAILED: ${error instanceof Error?error.message:String(error)}`);process.exitCode=1;}).finally(async()=>pool.end());
