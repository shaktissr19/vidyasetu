...
const PACK_DIR = path.resolve(
  __dirname,
  '../../../content/class-8/science/force-and-pressure/effects-of-force',
);

const RESOURCE_SLUG = 'class-8-science-effects-of-force-v1';
const PACK_ID = 'VS-C8-SCI-FP-EFFECTS-V1';
...
function validatePack(manifest: PackManifest, bank: QuestionBank): void {
  if (manifest.packId !== PACK_ID) throw new Error(`Unexpected Effects of Force packId: ${manifest.packId}`);
  ...
  if (manifest.sourceCode !== 'VIDYASETU_ORIGINAL' || bank.sourceCode !== 'VIDYASETU_ORIGINAL') {
    throw new Error('Effects of Force installer only accepts VIDYASETU_ORIGINAL content');
  }
  if (manifest.licence !== 'VIDYASETU_ORIGINAL' || bank.licence !== 'VIDYASETU_ORIGINAL') {
    throw new Error('Effects of Force installer only accepts VIDYASETU_ORIGINAL licence');
  }
  if (bank.packId !== manifest.packId) throw new Error('Question bank packId does not match manifest');
  if (!manifest.languages.includes('en') || !manifest.languages.includes('hi')) throw new Error('Effects of Force pack must declare English and Hindi');
  if (manifest.gradeCodes.length !== 1 || manifest.gradeCodes[0] !== 'CLASS_8') throw new Error('Effects of Force v1 is locked to CLASS_8');
  if (manifest.boardCodes.length !== 1 || manifest.boardCodes[0] !== 'COMMON') throw new Error('Effects of Force v1 is locked to COMMON cross-board scope');
  if (bank.questions.length !== 12) throw new Error(`Effects of Force v1 requires exactly 12 questions; found ${bank.questions.length}`);
...
async function requireSuperAdmin(userId: UUID): Promise<void> {
  ...
  if (user.role !== 'SUPER_ADMIN') throw new Error(`Effects of Force installation requires SUPER_ADMIN; received ${user.role}`);
}
...
async function ensureDraftResource(
  ...
  if (existing) {
    ...
    const created = await createLearningResource(
      {
        title: 'Effects of Force: what can a force change?',
        titleHi: 'बल के प्रभाव: बल क्या-क्या बदल सकता है?',
        summary: 'See how force can start or stop motion, change speed or direction, and change shape through familiar examples and a safe observation activity.',
        summaryHi: 'परिचित उदाहरणों और सुरक्षित गतिविधि से समझें कि बल गति शुरू या रोक सकता है, चाल या दिशा बदल सकता है और आकार भी बदल सकता है।',
        bodyMarkdown: bodyEn,
        ...
        attributionText: 'VidyaSetu Original — Class 8 Science Effects of Force learning pack',
        isOfflineReady: true,
...
function assessmentSlug(assetId: string): string {
  if (assetId === 'VS-EFFECTS-PRACTICE-01') return 'class-8-science-effects-of-force-practice-v1';
  if (assetId === 'VS-EFFECTS-MASTERY-01') return 'class-8-science-effects-of-force-mastery-v1';
  return `class-8-science-effects-of-force-${assetId.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
}
...
async function ensureDraftAssessments(
  ...
  for (const item of manifest.sequence.filter((entry) => entry.type === 'QUIZ')) {
    ...
    if (existing) {
      ...
      const isMastery = item.assetId === 'VS-EFFECTS-MASTERY-01';
      const created = await createAssessment(
        {
          ...
          summary: isMastery
            ? 'A concept-level mastery check on motion, speed, direction, shape and multi-effect reasoning.'
            : 'Low-stakes practice on the main observable effects of force before application.',
    ...
    await query(
      ...
      [
        assessmentId,
        item.assetId === 'VS-EFFECTS-MASTERY-01'
          ? 'गति, चाल, दिशा, आकार और एक से अधिक प्रभावों पर तर्क की अवधारणा-स्तर की महारत जाँच।'
          : 'अनुप्रयोग से पहले बल के मुख्य दिखाई देने वाले प्रभावों की कम-दबाव वाली अभ्यास जाँच।',
      ],
    );
...
async function main(): Promise<void> {
  ...
  console.log(`Effects of Force pack validated: ${manifest.packId} (${manifest.version})`);
  ...
  console.log('EFFECTS OF FORCE PACK STAGED SUCCESSFULLY — DRAFT ONLY');
...
main()
  .catch((error: unknown) => {
    ...
    console.error(`EFFECTS OF FORCE PACK INSTALL FAILED: ${message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
    await pool.end();
